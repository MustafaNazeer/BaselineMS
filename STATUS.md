...
**Phase 4: Gait Test Module Integration.** Not started. Mandatory check-in protocol must run before Phase 4 work begins. Phase 4 prep includes the rotationVector null fallback finding (Resume notes new item 9 below) which the Sensor Integration Engineer must address either by populating `ImuSample.rotationVector` from the from scratch Madgwick filter in the signals layer, or by adding a Madgwick fallback inside `GaitPipeline.rotateToWorld`. The plan for Phase 4 will be written at the start of the next session per `docs/plan.md` Phase 4 entry, after the boundary check-in.
