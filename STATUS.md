...
**Phase 3: Gait Signal Processing.** In progress (started 2026-05-07, mandatory check in answered: user reported 4% used, 3 hours until reset, continue). Detailed plan at `docs/plans/phase-3-gait-signal-processing.md`. PM triage of Phase 2 carryover: all 5 Patient Advocate findings, the Code Reviewer minor finding, and the abandoned session lifecycle bug (Resume notes items 1, 2, and 8 below) are deferred to Phase 11 (Accessibility, Beta, Polish) so Phase 3 stays focused on the pure DSP module within its 80 to 95 percent window estimate. The only Phase 2 carryover handled inside Phase 3 was the SPEC.md Section 6.1 miss rate operational definition (Resume notes item 5), which is now amended.
...
| 3 | Gait Signal Processing | in progress |  | est. 80 to 95% | Pure DSP module: Madgwick filter, Butterworth low pass, step detection, ZUPT stride length, feature extraction. Unit tested against synthetic ground truth. Detailed plan at `docs/plans/phase-3-gait-signal-processing.md` |
...
5. **Clinical Validator Phase 2 caveat (RESOLVED 2026-05-07 in Phase 3 prep):** SPEC.md Section 6.1 used the phrase "miss rate" without an operational definition. The PM amended Section 6.1 at Phase 3 start with the Option A definition (miss rate = (non_alternating_taps + off_target_taps) / (valid_taps + non_alternating_taps + off_target_taps), computed across both rounds), citing the Clinical Validator's 2026-05-07 Phase 2 sign off note. Implementation in `app/src/main/java/com/mustafan4x/msbattery/battery/tap/TapFeatures.kt` was verified against the formula in this prep pass.
...
8. **Abandoned session lifecycle finding (from 2026-05-07 user walkthrough):** when the user navigates away from `SessionRunnerScreen` via the system back button (instead of the Cancel TopAppBar action plus the AlertDialog confirmation), the orchestrator's `cancel()` is never called and the in flight `SessionEntity` row stays in the database with `completedAtEpochMs = null`, so the History list on Home renders it as "In progress" forever. The Patient Advocate Phase 2 review did not catch this because the review was source level only. Two reasonable fixes (Phase 3 Android Engineer to pick): (a) treat system back during a Running session as equivalent to the Cancel button (invoke the cancel confirmation dialog) so the user has a single discard path; or (b) on `SessionRunnerScreen` `DisposableEffect` cleanup, if the orchestrator is in `Running` state, call `orchestrator.cancel()` automatically. Option (a) is more honest but requires intercepting back; option (b) is simpler but silently discards work the user may have wanted to keep. The PM has no preference; the Patient Advocate should weigh in if option (b) is chosen. Owner: Android Engineer plus Patient Advocate ratification.
