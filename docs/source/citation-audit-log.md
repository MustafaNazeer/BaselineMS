...
## Phase 3 audit, DSP method citations, 2026-05-07

This section audits every citation that appears in the Phase 3 prose deliverables. The Phase 3 deliverables under audit are the from scratch Madgwick ADR (`docs/adr/0002-madgwick-from-scratch.md`), the synthetic IMU fixture library reference (`docs/qa/fixtures.md`), and the citation comments in the DSP source files (`app/src/main/java/com/mustafanazeer/baselinems/dsp/Madgwick.kt` and `app/src/main/java/com/mustafanazeer/baselinems/dsp/Zupt.kt`).

The audit method is to verify each Phase 3 prose claim about a cited paper traces to an entry in `docs/source/clinical-references.md` and that the prose claim accurately reflects what the bibliography entry says. The bibliography itself was previously audited against primary sources in the Phase 0 audit and the Phase 0 to Phase 1 cleanup re audit above; this Phase 3 audit confirms the Phase 3 prose is consistent with the audited bibliography. Per the Phase 3 plan (Task 16), this audit does not re fetch primary sources.

### P3.1. Madgwick 2010 in `docs/adr/0002-madgwick-from-scratch.md`, References section

- Artifact and line: `docs/adr/0002-madgwick-from-scratch.md` line 58 (the References list bullet "Madgwick S O H. 2010. An efficient orientation filter for inertial and inertial / magnetic sensor arrays. University of Bristol technical report. The IMU only variant in eq 11 to eq 33 is the basis for this implementation.").
- Cited claim: the project implements the IMU only variant of the Madgwick 2010 orientation filter, as described in equations 11 through 33 of the University of Bristol technical report.
- Source consulted: `docs/source/clinical-references.md` line 35 ("Madgwick 2010, 'An efficient orientation filter for inertial and inertial/magnetic sensor arrays.' The orientation filter used in Phase 3 of this project. Open source reference implementations exist on GitHub.") and Phase 0 audit log entry A.9 above ("Madgwick SOH. An efficient orientation filter for inertial and inertial/magnetic sensor arrays. Internal Report, University of Bristol. 2010. Verdict: confirmed.").
- Verdict: PARTIAL. The paper title, year, and venue (University of Bristol technical report) match the bibliography entry and the Phase 0 audit verdict A.9 exactly. The ADR's specific claim that the implementation is based on equations 11 to 33 of the original technical report is more specific than anything the bibliography or the Phase 0 audit recorded. The Phase 0 audit confirmed the paper exists and confirmed it is the foundational orientation filter work cited in the project; it did not record what the specific equation numbers are in the technical report. Per the "memory is not proof" rule the Citation Auditor does not assert from memory whether the IMU only variant in the 2010 technical report is in fact at equations 11 to 33. The substantive claim that the project's Madgwick filter is the IMU only variant of the published filter is consistent with the bibliography; the equation number reference is an additive specificity claim that this audit cannot verify against the bibliography alone.
- Recommended correction: either (a) the ADR drops the "eq 11 to eq 33" specificity and refers more generally to "the IMU only variant of the published filter," which is fully supported by the existing bibliography entry, or (b) the Citation Auditor performs a Phase 5 follow up with direct fetch of the Madgwick 2010 technical report to verify the equation numbers before any user facing artifact (README, in app About screen) repeats the equation reference. Option (a) is the lower friction fix for Phase 3 close. The PARTIAL verdict does not block the Phase 3 commit because the equation reference is internal to an ADR and is not currently propagated to any user facing artifact; it is recorded for the project's future cleanup pass.

### P3.2. Madgwick 2010 in `app/src/main/java/com/mustafanazeer/baselinems/dsp/Madgwick.kt` source comment

- Artifact and lines: `app/src/main/java/com/mustafanazeer/baselinems/dsp/Madgwick.kt` lines 6 to 22 (the class level KDoc).
- Cited claims, in order:
  1. "Madgwick (2010) IMU orientation filter, implemented from scratch per SPEC.md Section 7.1 step 2 ('teaching exercise'). Reference: Madgwick S O H. 2010. An efficient orientation filter for inertial and inertial / magnetic sensor arrays. University of Bristol technical report. The IMU only variant (no magnetometer) is used here." (Bibliographic claim.)
  2. "The default beta gain (0.1) is the value Madgwick reports as a reasonable starting point in the paper." (Substantive paper content claim about a recommended beta value.)
- Source consulted: `docs/source/clinical-references.md` line 35 (the Madgwick 2010 entry) and Phase 0 audit log entry A.9 above.
- Verdict: PARTIAL. Claim 1 (the bibliographic reference) matches the bibliography entry and the Phase 0 audit A.9 verdict. Claim 2 (that the paper recommends 0.1 as a reasonable starting beta) is a substantive paper content claim that the bibliography does not record. The Phase 0 audit verified the paper exists and is the orientation filter the project cites, but it did not record what the paper says about default beta values. Per "memory is not proof" the Citation Auditor does not assert whether 0.1 is the value the paper recommends without re fetching the primary source.
- Recommended correction: either (a) the source comment drops the "Madgwick reports as a reasonable starting point" attribution and refers to 0.1 as "the project's chosen default starting value," which removes the unverified paper content claim and is consistent with what the bibliography supports, or (b) the Citation Auditor performs a Phase 5 follow up with direct fetch of the Madgwick 2010 paper to verify the recommended beta range before the source comment is repeated in any user facing artifact. Option (a) is the lower friction fix; the source comment is internal to the DSP package and not propagated to any user facing artifact, so the PARTIAL verdict does not block the Phase 3 commit.

### P3.3. Skog et al. 2010 in `app/src/main/java/com/mustafanazeer/baselinems/dsp/Zupt.kt` source comment

- Artifact and lines: `app/src/main/java/com/mustafanazeer/baselinems/dsp/Zupt.kt` lines 3 to 11 (the class level KDoc).
- Cited claim: "Skog et al. 2010 ZUPT method (Skog I, Handel P, Nilsson J O, Rantakokko J. 2010. Zero velocity detection: an algorithm evaluation. IEEE Trans Biomed Eng 57(11):2657 to 2666). Given a forward (world frame Y) linear acceleration time series and a list of mid stance sample indices, integrate forward acceleration twice using trapezoidal integration with the velocity reset to zero at each mid stance."
- Source consulted: `docs/source/clinical-references.md` line 36 ("Skog et al. 2010, *IEEE Transactions on Biomedical Engineering*. Zero velocity update (ZUPT) method for stride length integration without GPS or external markers.") and Phase 0 audit log entry A.10 above ("Skog I, Handel P, Nilsson J-O, Rantakokko J. Zero-velocity detection: an algorithm evaluation. IEEE Transactions on Biomedical Engineering. 2010;57(11):2657-2666. doi:10.1109/TBME.2010.2060723. Verdict: confirmed.").
- Verdict: VERIFIED. The full author list, year, journal, volume, issue, and page range in the source comment all match the Phase 0 audit verdict A.10 exactly. The substantive claim that the paper is the ZUPT method for stride length integration matches the bibliography's relevance summary. The Phase 0 audit's caveat that the Skog et al. 2010 evaluation is foot mount specific while this project uses a hip mounted (front pocket) phone is preserved by the SPE in `docs/plans/phase-3-gait-signal-processing.md` as an implementation concern; that caveat does not affect the citation itself.

### P3.4. Bea T et al. 2025 in `docs/qa/fixtures.md`

- Artifact and lines: `docs/qa/fixtures.md` line 72 ("The Bea et al. 2025 systematic review (`Journal of Functional Morphology and Kinesiology` 10(2):133, DOI 10.3390/jfmk10020133) reports validity for spatial parameters of r = 0.42 to 0.90 against gold standard reference systems and standard error of measurement of 2.1 to 7.8 percent of walking distance (in two of 54 reviewed studies); these ranges are used as realism context, not as parameter values...") and line 246 (the `noisyMsNormal` realism rationale: "see Bea et al. 2025 for the broader noise context across 54 reviewed studies").
- Cited claims:
  1. Journal: *Journal of Functional Morphology and Kinesiology* 10(2):133.
  2. DOI: 10.3390/jfmk10020133.
  3. r = 0.42 to 0.90 for spatial parameter validity against gold standard systems.
  4. SEM 2.1 to 7.8 percent of walking distance, reported in 2 of 54 studies.
  5. The 54 study count.
- Source consulted: `docs/source/clinical-references.md` line 34 (the Bea 2025 entry) and Phase 0 to Phase 1 cleanup re audit log entry R.2 above (which directly verified the bibliographic detail and the four numerical claims against the open access PMC full text).
- Verdict: VERIFIED. All five claims match the Phase 0 to Phase 1 cleanup re audit verdict R.2 exactly. The fixtures.md framing characterises the SEM range as "of walking distance" rather than as "stride length specific," which is the faithful framing the R.2 re audit recommended. The fixtures.md sentence "these ranges are used as realism context, not as parameter values" further constrains the Bea 2025 numbers to a realism context role rather than a load bearing parameter role, which is consistent with the bibliography's "thin literature anchor" framing.

### P3.5. Givon et al. 2009 in `docs/qa/fixtures.md`

- Artifact and lines: `docs/qa/fixtures.md` line 72 ("The Givon et al. 2009 (`Gait Posture` 29(1):138 to 142) MS versus healthy control gait reference values are the primary clinical anchor. Givon et al. report MS step length 45.3 cm and cadence 94.4 steps per minute, versus healthy control step length 72.1 cm and cadence 115.2 steps per minute. Stride length is twice step length per gait literature convention, so the Givon healthy control stride length is 1.442 m and the Givon MS stride length is 0.906 m."), line 95 (the `healthyControlNormal` realism rationale), line 120 (the `msTypicalNormal` realism rationale), and line 282 (the References section, "Givon U, Zeilig G, Achiron A. 2009. Gait analysis in multiple sclerosis: characterization of temporal spatial parameters using GAITRite functional ambulation system. `Gait Posture` 29(1):138 to 142. Source of MS versus healthy control cadence and step length reference values used in the `healthyControlNormal` and `msTypicalNormal` fixtures.").
- Cited claims:
  1. Authors: Givon U, Zeilig G, Achiron A.
  2. Year: 2009.
  3. Journal: *Gait Posture* 29(1):138 to 142.
  4. MS step length 45.3 cm, MS cadence 94.4 steps per minute.
  5. Healthy control step length 72.1 cm, healthy control cadence 115.2 steps per minute.
  6. Stride length is twice step length per gait literature convention; therefore Givon healthy control stride length 1.442 m and Givon MS stride length 0.906 m.
- Source consulted: `docs/source/clinical-references.md` line 88 (the Phase 0 sign off note for the Gait Test, which states the values "are from Givon et al. 2009 (*Gait Posture* 29(1):138 to 142)") and Phase 0 audit log entries B.9 and D.6 above (which directly verified the Givon attribution and corrected the prior misattribution to Comber 2017).
- Verdict: VERIFIED. The author list, year, journal, volume, issue, page range, and the four raw cm and steps per minute values all match the Phase 0 audit verdict B.9 / D.6 exactly. The stride length conversion (twice step length) is a gait literature convention, not a Givon paper claim, and is correctly framed by fixtures.md as "per gait literature convention" rather than as a Givon assertion. The derived stride lengths (1.442 m and 0.906 m) follow algebraically from the cited step lengths and the convention.

### P3.6. Comber et al. 2017 in `docs/qa/fixtures.md`

- Artifact and lines: `docs/qa/fixtures.md` line 284 (the References section, "Comber et al. 2017, `Gait Posture`. Meta analysis of MS gait spatiotemporal parameters. Complementary to Givon 2009; cited in `SPEC.md` Section 7.1.1 as a complementary source for the per stride feature design choice.").
- Cited claims:
  1. Authors: Comber et al.
  2. Year: 2017.
  3. Journal: *Gait Posture*.
  4. The paper is a meta analysis of MS gait spatiotemporal parameters.
  5. Complementary, not primary, source.
- Source consulted: `docs/source/clinical-references.md` line 88 (the Phase 0 sign off note: "Comber et al. 2017 (*Gait Posture*) reports standardized mean differences in a meta analysis and is a complementary, not primary, source for these specific raw values.") and Phase 0 audit log entry B.9 above ("Comber L, Galvin R, Coote S. Gait deficits in people with multiple sclerosis: A systematic review and meta-analysis. Gait Posture. 2017;51:25-35. doi:10.1016/j.gaitpost.2016.09.026. Verdict: confirmed for inclusion.").
- Verdict: VERIFIED. The author group (Comber et al.), year (2017), journal (*Gait Posture*), the paper type (meta analysis of MS gait spatiotemporal parameters), and the complementary source framing all match the bibliography and the Phase 0 audit B.9 verdict. The fixtures.md entry does not assert raw cm or steps per minute values for Comber, which is correct: the Phase 0 audit D.6 veto was specifically that Comber 2017 reports SMDs and not raw cm or steps per minute values. The fixtures.md framing is faithful to that veto.

### P3.7. Buckley et al. 2020 in `docs/adr/0002-madgwick-from-scratch.md`, References section

- Artifact and line: `docs/adr/0002-madgwick-from-scratch.md` line 61 (the References list bullet "`docs/source/clinical-references.md` smartphone gait analysis section, which lists Madgwick 2010 alongside Skog et al. 2010 ZUPT and Buckley et al. 2020 as the project's primary DSP method references.").
- Cited claim: that `docs/source/clinical-references.md` currently lists Buckley et al. 2020 alongside Madgwick 2010 and Skog et al. 2010 as a primary DSP method reference.
- Source consulted: `docs/source/clinical-references.md` line 34 (the smartphone gait analysis section, which now lists Bea T et al. 2025 in place of the previously cited Buckley 2020) and Phase 0 audit log entry A.8 above ("Buckley et al. 2020, Sensors. Verdict: not found. Veto exercised."), plus the Phase 0 to Phase 1 cleanup re audit log entry R.2 above (which verified Bea T et al. 2025 as the replacement and confirmed the bibliography no longer cites Buckley 2020 anywhere except in historical Phase 0 sign off notes).
- Verdict: MISMATCH. The ADR's References section asserts that `docs/source/clinical-references.md` currently lists Buckley et al. 2020 as a primary DSP method reference. This is not what the current bibliography says. The bibliography's smartphone gait analysis section was updated in the Phase 0 to Phase 1 cleanup pass to replace the previously cited (and unlocatable) Buckley 2020 *Sensors* entry with Bea T et al. 2025 *Journal of Functional Morphology and Kinesiology* per Citation Auditor verdict A.8 and the Clinical Validator's Phase 0 to Phase 1 transition note. The ADR was written after the Phase 0 to Phase 1 cleanup but appears to reference the pre cleanup state of the bibliography. The Phase 0 audit verdict A.8 specifically vetoed the Buckley 2020 *Sensors* citation; reintroducing it in any project artifact, including an ADR's References section, is a regression.
- Recommended correction: replace "Buckley et al. 2020" in the ADR References section with "Bea T et al. 2025" so the ADR accurately reflects the current bibliography. The corrected wording is: "`docs/source/clinical-references.md` smartphone gait analysis section, which lists Madgwick 2010 alongside Skog et al. 2010 ZUPT and Bea T et al. 2025 (replacing the previously cited Buckley et al. 2020 per Citation Auditor verdict A.8) as the project's primary DSP method references." Alternatively the ADR can drop the Buckley reference entirely and refer to "the project's primary DSP method references in the smartphone gait analysis section of `docs/source/clinical-references.md`" without naming a third paper. Either correction restores consistency with the audited bibliography.
- This MISMATCH is the only finding from the Phase 3 audit that requires a content change to a Phase 3 deliverable. The PM should dispatch the Documentation Engineer (or the SPE who authored the ADR) to update line 61 of `docs/adr/0002-madgwick-from-scratch.md` before any Phase 3 artifact is referenced as authoritative downstream.

### Phase 3 audit summary

- Audits performed: 7.
- Verdict counts: VERIFIED 4 (P3.3 Skog Zupt.kt, P3.4 Bea T 2025 fixtures.md, P3.5 Givon 2009 fixtures.md, P3.6 Comber 2017 fixtures.md); PARTIAL 2 (P3.1 Madgwick 2010 ADR equation numbers, P3.2 Madgwick 2010 Madgwick.kt beta recommendation); MISMATCH 1 (P3.7 Buckley et al. 2020 in ADR References, regressed against the Phase 0 to Phase 1 cleanup that replaced Buckley with Bea T 2025).
- Headline: APPROVED WITH MINOR FINDINGS. One MISMATCH (P3.7) requires a one line correction to `docs/adr/0002-madgwick-from-scratch.md` line 61 to restore consistency with the audited bibliography; the two PARTIAL findings (P3.1 and P3.2) flag specificity claims that the bibliography does not pre audit and that may be tightened in a Phase 5 follow up but do not block the Phase 3 commit because the claims are internal to an ADR and a source comment, not propagated to any user facing artifact yet.
- Items the Citation Auditor was uncertain about and did not assert in this Phase 3 audit:
  1. Whether the IMU only variant in the Madgwick 2010 University of Bristol technical report is in fact at equations 11 to 33 (P3.1). The bibliography does not record equation numbers; the Citation Auditor did not re fetch the primary source in this Phase 3 audit per the Phase 3 plan Task 16 scope, which explicitly states the audit confirms the Phase 3 prose is consistent with the audited bibliography rather than re fetching primary sources.
  2. Whether the Madgwick 2010 paper recommends 0.1 as a reasonable starting beta value (P3.2). Same constraint as item 1.
- Vetoes exercised: none new in this Phase 3 audit. The pre existing Phase 0 audit veto on Buckley et al. 2020 (verdict A.8) is reaffirmed; the P3.7 MISMATCH is the consequence of that veto being violated by the ADR's reference to Buckley 2020.

Citation Auditor, Phase 3 audit, 2026-05-07.
