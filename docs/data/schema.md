...
```kotlin
// /home/mustafa/src/MS-Battery/app/src/main/java/com/mustafan4x/msbattery/data/Migrations.kt
...
In `/home/mustafa/src/MS-Battery/app/build.gradle.kts`, after line 47 (the closing brace of the `android { ... }` block) and before line 49 (the `dependencies { ... }` block opening), add:
...
In `/home/mustafa/src/MS-Battery/app/src/main/java/com/mustafan4x/msbattery/data/AppDatabase.kt`, change:
...
The DBA picks the KSP arg form. Both produce the same `app/schemas/com.mustafan4x.msbattery.data.AppDatabase/<version>.json` artifact; the choice is stylistic and operational.
...
In `/home/mustafa/src/MS-Battery/app/build.gradle.kts`, after the `android { ... }` block (currently closes at line 47) and before the `dependencies { ... }` block (currently opens at line 49), add:
