...
- Mean step time across both feet: 60 / 90 = 0.667 seconds.
- Dominant foot mean step time: 0.667 * 1.30 = 0.867 seconds.
- Non dominant foot mean step time: 0.667 * (2 - 1.30) = 0.467 seconds.
- Stride asymmetry index (symmetric mean denominator): (0.867 - 0.467) / 0.667 = 0.60.
- Total distance: 45 * (1.05 / 2) = 23.6 m.

Realism rationale: this fixture represents the high end of asymmetry the project plausibly sees. Cadence 90 is slightly below the Givon MS mean, and stride length 1.05 m is above the Givon MS mean of 0.906 m, reflecting a participant whose primary gait deficit is asymmetry rather than overall slowing. The asymmetry ratio of 1.30 means the dominant foot's mean step time is 30 percent longer than the non dominant foot's. The 8 percent step time CV reflects the higher variability typical of asymmetric gait. The dominant foot mean step time of 867 ms is above the pipeline maximum inter peak distance ceiling of 800 ms, so a real recording at this asymmetry level would have some dominant foot steps rejected as pauses. This is by design: pipeline tests using this fixture verify the pipeline's behavior on the boundary of its envelope.

Caveats: the 867 ms dominant step time will be flagged by the pipeline's max inter peak distance constraint. This is a feature, not a defect; the pipeline correctly excludes excessively long inter step intervals. Tests using this fixture should expect a partial step count rather than the full 45.
...
5. **Lateral sway and forward propulsion amplitudes.** The generator uses fixed amplitudes (1.0 m per second squared on world X for sway, 0.5 m per second squared on world Y for propulsion). These are reasonable orders of magnitude but are not calibrated against published gait acceleration norms or against real recordings. They are placeholder amplitudes that produce a recognizable walking signature; the pipeline's left versus right pairing logic depends on the sign of the lateral acceleration, not on its absolute magnitude, so the placeholder amplitude is sufficient for the Phase 3 tests.
6. **Stride length variability.** The generator produces strides of constant length (the parameter `strideLengthMeters` is the mean and is also the value for every stride). Real walkers have stride to stride length variation that the project's pipeline should ultimately measure. The current fixture library does not exercise stride length variability; if a Phase 3 test needs to verify the pipeline's stride length CV output, a new parameter (`strideLengthCv`) would need to be added to the generator. This is recorded as a gap for the Test Fixture Engineer's Phase 5 review.
7. **`rotationVector` quaternion convention.** The generator stores `rotationVector` as the world to device rotation (i.e., `rotationVector.rotate(worldVec) = deviceVec`). This is opposite of Android's `TYPE_ROTATION_VECTOR` convention, which is device to world (`q.rotate(deviceVec) = worldVec`). The synthetic library is internally consistent, and the `SyntheticImuTest.kt` round trip test (`linearAcceleration plus gravity equals raw accelerometer in the device frame`) verifies the convention in use. Downstream Phase 4 code that consumes `ImuSample.rotationVector` from real Android sensors will encounter the opposite convention. The Test Fixture Engineer recommends the PM resolve this convention divergence before Phase 4 starts: either flip the synthetic generator's convention to match Android (and update the round trip test), or document the divergence as load bearing in the `ImuSample.kt` source comment so the Sensor Integration Engineer accounts for it when wiring the real sensor stream.
