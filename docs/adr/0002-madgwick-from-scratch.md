...
Implement the Madgwick filter from scratch in Kotlin under `app/src/main/java/com/mustafan4x/msbattery/dsp/Madgwick.kt`.

The implementation is small (under 100 lines), parallels the published paper one to one, and preserves the project's principle of keeping the gait pipeline pure JVM Kotlin with no third party dependencies beyond the Kotlin standard library and `kotlin.math`. Two unit tests in `app/src/test/java/com/mustafan4x/msbattery/dsp/MadgwickTest.kt` exercise the static gravity convergence case and the pure gyro integration case at zero beta.
...
- `docs/source/clinical-references.md` smartphone gait analysis section, which lists Madgwick 2010 alongside Skog et al. 2010 ZUPT and Buckley et al. 2020 as the project's primary DSP method references.
